...
export default function LocatiesPage() {
  ...
  const denHaagRegion = locations.filter(loc =>
    ['den-haag', 'scheveningen', 'wassenaar', 'rijswijk', 'voorburg', 'leidschendam'].includes(loc.slug)
  )
  const leidenRegion = locations.filter(loc =>
    ['leiden', 'oegstgeest', 'voorschoten', 'leiderdorp'].includes(loc.slug)
  )
  const airports = locations.filter(loc =>
    ['schiphol', 'rotterdam-the-hague-airport'].includes(loc.slug)
  )
  const otherLocations = locations.filter(loc =>
    ['delft', 'zoetermeer', 'katwijk', 'noordwijk'].includes(loc.slug)
  )
        ...
        <div className="container-custom relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 mb-6">
              ...
              <span className="text-sm font-semibold text-white uppercase tracking-wider">Onze Locaties</span>
            </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-primary-400 font-semibold">
                      Vanaf €{location.popularRoutes[0]?.priceFrom || 79},-
                    </p>
                  </div>
                </div>
              </Link>
        ...
        <div className="container-custom">
          <div className="text-center mb-12">
            ...
            <p className="text-secondary-600 max-w-2xl mx-auto">
              Premium taxi service in Den Haag, Scheveningen, Wassenaar en omliggende gemeenten
            </p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {denHaagRegion.map((location, index) => (
              <Link
                key={location.slug}
      ...
      <section className="py-20 bg-secondary-50">
        <div className="container-custom">
          <div className="text-center mb-12">
            ...
            <p className="text-secondary-600 max-w-2xl mx-auto">
              Betrouwbare taxi service in Leiden, Oegstgeest, Voorschoten en Leiderdorp
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
            {leidenRegion.map((location, index) => (
              <Link
                key={location.slug}
                href={`/locations/${location.slug}`}
                className="group p-6 bg-white rounded-xl border border-secondary-200 hover:border-purple-300 hover:shadow-lg transition-all text-center"
              >
      ...
      <section className="py-20 bg-white">
        <div className="container-custom">
          <div className="text-center mb-12">
            ...
            <p className="text-secondary-600 max-w-2xl mx-auto">
              Ook in Delft, Zoetermeer, Katwijk en Noordwijk zijn wij actief
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
            {otherLocations.map((location, index) => (
              <Link
                key={location.slug}
                href={`/locations/${location.slug}`}
                className="group p-6 bg-secondary-50 rounded-xl border border-secondary-200 hover:border-emerald-300 hover:shadow-lg transition-all text-center"
              >
      ...
      <section className="py-20 bg-gradient-to-r from-primary-600 to-primary-700">
        <div className="container-custom text-center">
          ...
          <p className="text-white/80 mb-8 max-w-2xl mx-auto">
            Wij rijden door heel Zuid-Holland. Neem contact op voor een offerte op maat.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <a
              href="tel:+31612345678"
